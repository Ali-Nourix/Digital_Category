...
* **94 products** (93 catalogue products + the "Quarry" page, `natanz-white-granite-rijen`), **357 product images** (171 of them project photos) and the site's texture download for **38 products**: 29 separate texture files, and for 9 products the texture is byte-identical to the main image and points to it. In total 386 image files, 61.3 MB.
* Project photos were completed on **2026-09-24** from the live site (see below and [PROJECTS.md](PROJECTS.md)).
...
* **Live site:** unreachable directly (see above). Only the projects section was re-read live (2026-09-24, through the translate proxy); everything else still reflects the last Wayback captures (Feb 2026). The live site now also has a stone that is not in this data, **Calacatta Viola** (`/product/calacatta-viola/`, project `/_project/calacatta-viola/`); see PROJECTS.md.
...
* **Products without any image:** none since 2026-09-24 (`black-tempest` has project photos from the live site, but still no slab or listing image).
...
* **Project photos:** most images of the project galleries were never archived. They were taken from the live site instead (2026-09-24): every gallery photo of the 40 live project pages is now in the stone's `images` as `kind: "project"`, except the one Calacatta Viola photo (no product folder). Where the original upload is over 2560 px or 3 MB, the site's `-scaled` copy is stored and the original is recorded in `original_upload`; two Jacaranda originals are also over the proxy's 15 MB limit, so their size comes from the JPEG header. See PROJECTS.md.
